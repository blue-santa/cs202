...
int main(int argc, char* argv[])
{
    ...
    vector<string> choiceList{"A", "B", "C", "S", "X", "P"};
    while (true) {
        clearConsole();
        ...
        // Print SVG Option
            ...
            SVGPrinter svg(filename, file_size_x, file_size_y, cave, bats, pit, wumpus);
            bool res = svg.printRooms();
            if (!res) {
                cout << "Error printing rooms" << endl;
                exit(0);
            }

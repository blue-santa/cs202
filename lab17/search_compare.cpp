...
int main() {
	while (true) {
		...
		vector<int> data_vals;
		for (int i {0}; i < size; i++) {
			int j = std::rand(); 
			data_vals.push_back(j); 
			data_keys.push_back(i);
		}
		...
		int thekey = data_keys.at(0);
		int thevalue;

		bool binRes = binarySearch(data_keys, data_vals, thekey, thevalue);
		...
		bool seqRes = sequentialSearch(data_keys, data_vals, thekey, thevalue);
